...
export function AppShell({
  navItems, activeItem, onNavSelect, profile,
  sectionTitle, topBar, actionButton, children,
}: AppShellProps) {
  ...
  const Sidebar = ({ onClose }: { onClose?: () => void }) => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-b border-white/5 shrink-0">
        <Link href="/" onClick={onClose}>
          <div className="flex items-center gap-2.5 cursor-pointer">
            <div className="w-8 h-8 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center text-base">🌿</div>
            <span className="font-display font-bold text-lg tracking-tight">Pausa</span>
          </div>
        </Link>
      </div>
        ...
        <div className="px-3 py-3 border-b border-white/5 shrink-0">
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl"
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold font-display leading-none mb-0.5" style={{ color: ring.color }}>
                {ring.name}
              </p>
              <p className="text-[11px] text-white/40 truncate leading-none">{profile.display_name}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {navItems.map((item) => (
          <div key={item.id}>
            {item.dividerAbove && <div className="my-2 border-t border-white/5 mx-2" />}
            <button
              onClick={() => { onNavSelect(item.id); if (item.href) window.location.href = item.href; onClose?.(); }}
    ...
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-white/5 shrink-0 bg-background/90">
          <button onClick={() => setMobileOpen(true)} className="p-1.5 rounded-lg text-muted-foreground hover:bg-white/5 transition-colors">
            <Menu className="w-5 h-5" />
          </button>
          {sectionTitle && <span className="text-sm font-display font-bold">{sectionTitle}</span>}
