...
function UserMenuClerk({ onNavClick }: { onNavClick?: () => void }) {
  ...
  const close = () => { setOpen(false); onNavClick?.(); };
...
export function Navbar() {
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? "bg-background/90 backdrop-blur-xl border-white/5 py-3" : "bg-transparent border-transparent py-4 md:py-6"
    )}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2.5 outline-none shrink-0">
          <img src="/pausa-logo.png" alt="Pausa logo" className="h-8 w-8 md:h-10 md:w-10 object-contain rounded-lg" />
        ...
        <div className="hidden md:flex items-center gap-4 lg:gap-6">
          {isHome && (
            <div className="flex items-center gap-5 text-sm font-medium text-muted-foreground">
              <button onClick={() => scrollTo("problem")} className="hover:text-primary transition-colors">Vision</button>
              <button onClick={() => scrollTo("pillars")} className="hover:text-primary transition-colors">Pillars</button>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-white/5 bg-background/95 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            {isHome && (
              <>
                <button onClick={() => scrollTo("problem")} className="w-full text-left px-3 py-2.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-white/5 rounded-lg transition-colors">Vision</button>
                <button onClick={() => scrollTo("pillars")} className="w-full text-left px-3 py-2.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-white/5 rounded-lg transition-colors">Pillars</button>
                <button onClick={() => scrollTo("how-it-works")} className="w-full text-left px-3 py-2.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-white/5 rounded-lg transition-colors">Engine</button>
