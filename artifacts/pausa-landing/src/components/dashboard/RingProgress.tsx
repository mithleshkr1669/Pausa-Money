...
const MILESTONES: Record<RingTier, { label: string; criteria: string }> = {
  1: {
    label: "A new beginning, planted with intent.",
    criteria: "You're here — welcome to the forest! 🌱",
  },
  2: {
    label: "Breaking ground, first growth emerging.",
    criteria: "Complete 1 goal + fill your Financial Profile",
  },
  3: {
    label: "Taking root, growing steadily.",
    criteria: "Complete 3 savings goals",
  },
  4: {
    label: "Thriving, sheltering others.",
    criteria: "Complete 6 savings goals",
  },
  5: {
    label: "Full canopy, shaping the landscape.",
    criteria: "Founder-selected — top community contributors",
  },
};
...
export function RingProgress({
  ...
  if (!isMaxTier) {
    if (currentTier === 1) {
      const done =
        (completedGoals >= 1 ? 1 : 0) + (hasFinancialProfile ? 1 : 0);
  ...
  return (
    <div className="glass-panel rounded-2xl p-6">
      <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-5">
        Your Ring Status
      </h3>
