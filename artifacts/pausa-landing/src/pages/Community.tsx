      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="text-xs font-mono" style={{ color: ring.color }}>
              {ring.name}
            </span>
            {story.city && (
...
function ShareStoryModal({
  ...
  const handleSubmit = async () => {
    if (
      ...
      setError("Please fill all story sections.");
      return;
    }
    setSubmitting(true);
  ...
  const fields: {
  ...
  }[] = [
    {
      key: "title",
      label: "Story Title",
      placeholder: "e.g. How I escaped debt at 26",
      textarea: false,
    },
    {
      key: "situation",
      label: "My Situation",
      placeholder: "I was 24, earning ₹38k, confused...",
      textarea: true,
    },
    {
      key: "challenge",
    ...
    },
    {
      key: "action",
      label: "What I Did",
      placeholder: "So I started by...",
      textarea: true,
    },
    {
      key: "result",
      label: "The Result",
      placeholder: "Now I have... and I feel...",
      textarea: true,
    },
    {
      key: "city",
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Stories inspire. Be specific and honest.
            </p>
          </div>
          <button
            onClick={onClose}
...
function CommunityInner() {
  ...
  const [savedReplies, setSavedReplies] = useState<
    (Answer & { saved_note?: string; post_title?: string })[]
  >([]);
  ...
  const loadContent = useCallback(async () => {
    ...
    setLoading(true);
    if (activeItem === "saved") {
      setSavedReplies(await getSavedReplies(user.id));
    } else if (activeItem === "stories") {
      setStories(await getStories());
    } else if (activeItem === "all") {
      setPosts(await getPosts(activeTag ?? undefined));
    } else if (activeItem === "mine") {
      setPosts(await getMyPosts(user.id));
    } else if (activeItem === "liked") {
      setPosts(await getLikedPosts(user.id));
    } else if (activeItem === "participated") {
      setPosts(await getParticipatedPosts(user.id));
    }
    setLoading(false);
          ...
          <div className="mb-5 flex items-start gap-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
            ...
            <div>
              ...
              <p className="text-xs text-yellow-400/70">
                Run migrations 001 → 002 → 003 → 004 in Supabase SQL Editor,
                then add keys to .env
              </p>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          stories.length === 0 ? (
            <div className="text-center py-16 glass-panel rounded-2xl">
              ...
              <button
                onClick={() => setShowStoryModal(true)}
                className="inline-flex items-center gap-2 bg-primary text-[#0A0A0C] font-semibold px-5 py-2.5 rounded-xl text-sm"
              >
              ...
              {savedReplies.map((r) => (
                <div key={r.id} className="glass-panel rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <RingBadge
                      tier={(r.profiles?.ring_tier ?? 1) as RingTier}
                      size="sm"
                    />
          ...
          <div className="text-center py-16 glass-panel rounded-2xl">
            ...
            {(activeItem === "mine" || activeItem === "all") &&
              isSupabaseConfigured && (
                <Link href="/community/ask">
