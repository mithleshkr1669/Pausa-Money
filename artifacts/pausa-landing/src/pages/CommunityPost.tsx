...
import { ChevronUp, ArrowLeft, Tag, Send, Loader2, AlertCircle, MessageCircle, BookMarked, BookmarkCheck, Users, Edit2, Trash2, Check, X } from "lucide-react";
import { Navbar } from "@/components/layout/Navbar";
import { Footer } from "@/components/layout/Footer";
import { RingBadge } from "@/components/community/RingBadge";
import { ProtectedRoute } from "@/components/auth/ProtectedRoute";
import {
  getPost, getAnswers, getComments, createAnswer, createComment,
  upvotePost, upsertProfile, saveReply, unsaveReply, isReplySaved,
  updatePost, deletePost, type Post, type Answer, type Comment, type RingTier,
} from "@/lib/community";
...
function AnswerCard({ answer, isSaved, onToggleSave, canSave, currentUserId }: {
  answer: Answer; isSaved: boolean; onToggleSave: () => void; canSave: boolean; currentUserId?: string;
}) {
  ...
  return (
    <div className={`rounded-2xl p-5 border ${answer.is_verified ? "border-primary/30 bg-primary/5" : "border-white/5 bg-card/40"}`}>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        {answer.is_ai ? (
          <span className="inline-flex items-center gap-1.5 text-xs font-mono px-2.5 py-1 rounded-full border border-primary/30 bg-primary/10 text-primary">
            <Bot className="w-3.5 h-3.5" />Pausa AI
          </span>
        ) : (
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {/* Save button */}
          {canSave && (
            <button onClick={onToggleSave} title={isSaved ? "Saved" : "Save reply"}
          ...
          {/* Upvote */}
          <button onClick={handleUpvote} disabled={voted || !currentUserId}
            ...
            <UpIcon className="w-3.5 h-3.5" />{votes}
          </button>
        </div>
      </div>

      {/* Body */}
      <div className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap pl-0.5">{answer.body}</div>
    </div>
  );
}
    ...
    <div className="flex gap-2.5 py-2.5 border-b border-white/4 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
          <RingBadge tier={ringTier} size="sm" />
          <span className="text-xs font-medium">{comment.profiles?.display_name ?? "Anonymous"}</span>
...
function CommunityPostInner() {
  ...
  useEffect(() => {
    ...
    Promise.all([getPost(params.id), getAnswers(params.id), getComments(params.id)]).then(([p, a, c]) => {
      setPost(p); setLocalVotes(p?.upvotes ?? 0); setAnswers(a); setComments(c); setLoading(false);
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (error || !newAnswer) { setAnswerError(error ?? "Failed to post."); return; }
  ...
  const handleSubmitComment = async () => {
    ...
    if (error || !newComment) { setCommentError(error ?? "Failed to post."); return; }
  ...
  const handleSaveEdit = async () => {
    if (!post || !user) return;
    setSavingEdit(true);
    const { error } = await updatePost(post.id, user.id, { title: editTitle.trim(), body: editBody.trim() });
    setSavingEdit(false);
    if (!error) { setPost((p) => p ? { ...p, title: editTitle.trim(), body: editBody.trim() } : p); setEditing(false); }
  };
  ...
  if (loading) return (
    <div className="min-h-screen bg-background"><Navbar />
      <div className="max-w-3xl mx-auto px-4 pt-28 pb-24 space-y-4">
        {[...Array(3)].map((_, i) => <div key={i} className="glass-panel rounded-2xl p-6 animate-pulse h-24" />)}
      </div>
    </div>
  );
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 md:pt-28 pb-24">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-2xl p-6 mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              {editing && (
                <>
                  ...
                  <button onClick={() => setEditing(false)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-muted-foreground hover:bg-white/5 border border-white/8">
          ...
          {showDeleteConfirm && (
            <div className="mt-4 flex items-center gap-3 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
              <p className="text-sm text-red-400 flex-1">Delete this post permanently?</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between mt-3 flex-wrap gap-3">
            <p className="text-xs text-muted-foreground italic">Educational only — not personalised financial advice.</p>
